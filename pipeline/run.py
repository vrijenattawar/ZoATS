...
def process_candidate(job: str, candidate_id: str, dry_run: bool = False) -> Dict[str, Any]:
    ...
    # Scoring
    scorer_script = WORKERS / "scoring" / "main.py"
...
def main() -> int:
    ...
    failed = len(cand_results) - complete - partial

    results["candidates_processed"] = len(cand_results)
    results["candidate_results"] = cand_results
    results["summary"] = {"total": len(cand_results), "complete": complete, "partial": partial, "failed": failed}
